    ...
    <form action="{{route('branch.store')}}" method="post">
        ...
        <div class="inputLabelContainer grid grid-cols-2 gap-0.5">
            <div class="cb_company_id flex flex-col">
                ...
                <select name="cb_company_id" id="cb_company_id">
                    @foreach ($companyData as $item)
                        <option value="{{ $item->id }}" {{ old('cb_company_id') == $item->id ? 'selected' : '' }}>{{ $item->nama_company }}</option>
                    @endforeach
                </select>
                @error('cb_company_id')
            ...
            <div class="tb_no_telp_branch flex flex-col">
                ...
                <input value="{{old('tb_no_telp_branch')}}" type="text" name="tb_no_telp_branch" id="tb_no_telp_branch">
